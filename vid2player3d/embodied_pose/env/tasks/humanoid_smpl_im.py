...
class HumanoidSMPLIM(HumanoidSMPL):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        if args.test:
            # Keep test behavior configurable instead of forcing Start every episode.
            test_state_init = cfg["env"].get("test_state_init", cfg["env"].get("stateInit", "Hybrid"))
            cfg["env"]["stateInit"] = test_state_init

            # Prefer CLI-provided test motion file when present.
            if hasattr(args, 'test_motion_file') and args.test_motion_file:
                cfg['env']['motion_file'] = args.test_motion_file
            elif 'test_motion_file' in cfg['env'] and cfg['env']['test_motion_file']:
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.cfg['env'].get('sample_first_motions', False):
            ...
            print(f"debug: 253: resampled, motion ids: {self._reset_ref_motion_ids}")
    ...
    def _expand_motion_list(self, motion_file):
        """
        Returns a list of .pth/.pt paths from:
          - directory (all *.pth)
          - yaml/yml (list/dict of files)
          - single file
        """
        # 1) directory
        if os.path.isdir(motion_file):
            files = sorted(glob.glob(os.path.join(motion_file, "*.pth")))
            return files

        # 2) yaml list
        if motion_file.endswith((".yaml", ".yml")):
            base_dir = os.path.dirname(os.path.abspath(motion_file))
            with open(os.path.join(os.getcwd(), motion_file), 'r') as f:
                data = yaml.load(f, Loader=yaml.SafeLoader)

            # Accept a few common yaml shapes:
            # A) {"motion_files": ["a.pth", "b.pth"], "motion_weights": [...]}
            files = data["motions"]
            # make paths absolute (relative paths are relative to yaml location)
            abs_files = [p["file"] if os.path.isabs(p["file"]) else os.path.join(base_dir, p["file"]) for p in files]
            return abs_files
    ...
    def _load_motion(self, motion_file):
        ...
        self.motion_lib_files = self._expand_motion_list(motion_file)

        if len(self.motion_lib_files) == 0:
            raise FileNotFoundError(f"No motion .pth files found for: {motion_file}")
