...
import vid2player.learning.common_agent as common_agent 
from vid2player.utils.common import AverageMeter, get_eta_str
...
class V2PAgent(common_agent.CommonAgent):
    ...
    def train(self):
        ...
        if pretrained_model_cp is not None and not self.config.get('load_checkpoint', False):
            print(f"Loading pretrained model from: {pretrained_model_cp}")
