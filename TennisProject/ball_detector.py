...
class BallDetector:
    def __init__(self, path_model=None, device='cuda'):
        self.model = BallTrackerNet(input_channels=9, out_channels=256).to(device)
        self.device = device
        if path_model:
    ...
    def infer_model(self, frames, start:int, end:int):
        ...
        for num in range(max(2,start), end): #tqdm
            ...
            x = torch.from_numpy(inp).to(self.device, dtype=torch.float32)
            out = self.model(x)
        ...
        if circles is not None:
            if prev_pred[0]:
                for i in range(len(circles[0])):
                    x_temp = circles[0][i][0]*scaleX
                    y_temp = circles[0][i][1]*scaleY
                    dist = distance.euclidean((x_temp, y_temp), prev_pred)
                    if dist < max_dist:
                        x, y = x_temp, y_temp
                        break                
            else:
                x = circles[0][0][0]*scaleX
                y = circles[0][0][1]*scaleY
        return x, y
