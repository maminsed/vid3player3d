...
class PersonDetector():
    ...
    def track_players(self, frames, matrix_all, start, end, filter_players=False):
        persons_top = []
        persons_bottom = []
        min_len = min(len(matrix_all),end-start)
        for num_frame in range(start, min_len+start): # tqdm
            img = frames[num_frame]
            if matrix_all[num_frame-start] is not None:
                inv_matrix = matrix_all[num_frame-start]
